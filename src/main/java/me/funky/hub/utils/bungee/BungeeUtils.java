package me.funky.hub.utils.bungee;
...
public class BungeeUtils implements PluginMessageListener {
    ...
    public static void refreshGlobalCount() {
        ...
        out.writeUTF("ALL");
        Player player = Iterables.getFirst(Tempest.getInstance().getServer().getOnlinePlayers(), null);
        if (player != null) player.sendPluginMessage(Tempest.getInstance(), "BungeeCord", out.toByteArray());
    }
    ...
    public static void sendToServer(Player p, String server) {
        ...
        p.sendPluginMessage(Tempest.getInstance(), "BungeeCord", out.toByteArray());
    }
